...
import vayAudio from "./assets/Vay.wav";
import AayAudio from "./assets/Aay.wav";
...
const App: React.FC = () => {
  ...
  const [points, setPoints] = useState(12222749365);
  const [clicks, setClicks] = useState<
    { id: number; x: number; y: number; xVid: number; yVid: number }[]
  >([]);
  const pointsToAdd = 11;
  const profitPerHour = 12640000;
  ...
  const handleCardClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    setClicks([
      ...clicks,
      {
        ...
        x: e.pageX,
        y: e.pageY,
        xVid: getRandomNumber(0, 50),
        yVid: getRandomNumber(0, 50),
      },
    ]);
  }, []);
  ...
  const generateZanetoo = (id: number, top: number, left: number) => {
    return (
      <div
        key={id}
      ...
      >
        <video className="" width="160" height="160" autoPlay muted loop>
          <source src={vayZanetoo} />
        </video>
        <audio autoPlay hidden  controls>
          <source src={id % 2===1?vayAudio:AayAudio} />
        </audio>
      </div>
    );
  };
  ...
  return (
    <div className="bg-black flex justify-center">
      <div className="w-full bg-black text-white h-screen font-bold flex flex-col max-w-xl relative overflow-hidden">
        {clicks.map((click) =>
          generateZanetoo(click?.id, click?.xVid, click?.yVid)
        )}
